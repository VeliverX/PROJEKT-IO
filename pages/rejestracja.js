...
export default function Home() {
  return (
    ...
    <div className="max-w-md w-full bg-white rounded p-10  space-y-6">
        <div className="mb-4">
            <p className="flex justify-center text-gray-600 text-3xl font-medium	">Zarejestruj się</p>
        </div>
        <div>
            <input className="w-full p-4 text-sm bg-gray-50 focus:outline-none border border-gray-200 rounded text-gray-600" type="text" placeholder="Nazwa Użytkwonika"></input>
        </div>
        <div>
        ...
        <div>
            <button className="w-full py-4 bg-yellow-400 bg-opacity-100 hover:bg-yellow-600 rounded text-sm font-bold text-gray-50 transition duration-200">Zarejestruj się</button>
        </div>
        <div>
            <p className="text-xs text-gray-500">Klikając przycisk „Zarejestruj się”, akceptujesz regulamin serwisu oraz politykę prywatności.</p>
        </div>
          
        <div className="flex items-center justify-between">
            <div className="flex flex-row items-center">
                <input type="checkbox" className="focus:ring-blue-500 h-4 w-4 text-blue-600 border-gray-300 rounded"></input>
                <label htmlhtmlFor="comments" className="ml-2 text-sm font-normal text-gray-600">Zapamiętaj hasło</label>
            </div>
        </div>
        <div> 
            <Link href="login">
            <a  className="flex justify-center text-sm text-gray-500 hover:underline" href="#">Mam już konto</a>  
            </Link> 
        </div>
    </div>
</section>
